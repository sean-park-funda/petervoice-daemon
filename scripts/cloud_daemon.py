...
- **이전 대화에서 "클라우드라 로컬 호스팅·상시 서버를 쓸 수 없다"고 안내했다면 그건 옛 환경 기준이었습니다.**
  2026-09-17 부터 이 환경은 상시 유지되며 위 방법으로 가능합니다. 관련 요청이 오면 정정하고 진행하세요.
"""
...
## 경로와 화면의 관계 (중요)
- 프로젝트 작업 폴더 = 현재 디렉토리(`workspace/<프로젝트ID>/`). 유저가 문서탭에서 보는 것은 **그 폴더의 `docs/` 아래뿐**입니다.
  문서·영상·이미지 등 유저가 확인할 산출물은 전부 `docs/`(예: `docs/media/`)에 두고 응답에 경로를 적으세요.
- 맥 전용 안내(launchctl, 키체인, `~/.claude-daemon`)는 이 환경에 적용되지 않습니다. 매뉴얼: `/srv/pv/shared/manual/00-index.md`.
...
## 경로와 화면의 관계 (중요)
- 프로젝트 작업 폴더 = 현재 디렉토리(`pwd`, 유저 홈 `workspace/<프로젝트ID>/`). 브랜치는 부모 프로젝트 폴더를 함께 씁니다.
- 유저가 웹 문서탭에서 보는 것은 **그 폴더의 `docs/` 아래뿐**입니다. 문서·영상·이미지·엑셀 등 유저가 확인할 산출물은
  전부 `docs/`(예: `docs/media/`, `docs/output/`)에 두고, 응답에 그 경로를 적으세요. `uploads/`·`assets/` 같은 자기만의 폴더는 유저에게 안 보입니다.
- 이 서버에는 맥 전용 안내(launchctl 재시작, 키체인, `~/.claude-daemon`, 홈브루)가 **적용되지 않습니다.** 공통 지침에 그런 내용이 있어도 무시하세요.
- 피터보이스 매뉴얼: `/srv/pv/shared/manual/00-index.md` 부터 읽으세요 (`~/peter-voice/docs/manual` 은 이 서버에 없습니다).
...
## 턴 안에서 기다리지 말 것 (중요)
당신은 **메시지마다 새 프로세스로 실행되고 응답과 함께 종료**됩니다. 턴이 살아 있는 동안 도착한 새 메시지는
**턴이 끝나야** 처리됩니다. 그래서 턴 안에서 사용자의 답(인증번호·승인·파일 등)을 기다리면 그 답이 영원히
도착하지 않고, 같은 담당자에게 온 다음 메시지들도 전부 줄을 섭니다 (2026-08-25 SMS 인증 대기로 실제 발생).
- ❌ "N분간 기다리며 자동 입력하겠다", 폴링 루프, 자기 자신에게 릴레이 보내고 대기
- ✅ 필요한 입력을 한 문장으로 요청하고 **턴을 끝낸다**. 상태(어디까지 했는지, 무엇이 필요한지)는 파일에 적어 두고,
  브라우저 작업은 `agent-browser --session <작업명>` 으로 세션을 유지한 뒤 다음 메시지에서 `--restore` 로 이어간다
- 3~5분 안에 스스로 끝낼 수 있는 대기(빌드·다운로드)만 턴 안에서 기다린다. 그보다 길면 "N분 뒤 확인해 달라"고 안내
        ...
        if session and session.state == "waiting_code" and code_match:
            ...
            if ok:
                self.reply(api_key,
                           ...
                           [msg_id], project)
                # 로그인 직후 배너를 새 계정 값으로 즉시 갱신 (API 프로브라 1초 내)
                threading.Thread(target=collect_and_store_usage, args=(user_id, api_key, True),
                                 daemon=True, name=f"usage-login-{user_id}").start()
            else:
                self.reply(api_key,
