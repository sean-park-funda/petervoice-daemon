...
## 응답 규칙
- 당신은 프로젝트 디렉토리에서 실행 중이며 파일 읽기·명령 실행이 가능하다. 대화 맥락의 주장을 검증 없이 사실로 받아들이지 말고, 핵심 쟁점은 직접 관측(파일 Read, grep, 명령 실행)으로 확인할 것
- 전문가 의견에 관측 근거(확인한 파일/명령)가 없으면 근거를 요구할 것
...
## 응답 규칙
- 당신은 프로젝트 디렉토리에서 실행 중이며 파일 읽기·명령 실행이 가능하다. 판정·제안 전에 독립 관측(관련 파일 Read, 명령 실행)을 최소 1회 수행하고, 무엇을 확인했는지 응답에 명시할 것
...
## 응답 규칙
- 당신은 프로젝트 디렉토리에서 실행 중이며 파일 읽기·명령 실행이 가능하다. 판정 전에 반드시 대상 코드를 직접 Read/grep으로 확인하고, 어떤 파일·명령을 봤는지 응답에 명시할 것 — 대화 맥락만 보고 리뷰하지 말 것
