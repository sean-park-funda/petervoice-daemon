...
**CRITICAL**:
- **API Version**: Use `2022-06-28`. Version `2025-09-03` causes intermittent 401 errors on POST/PATCH requests.
- **Environment Variable Access**: `$NOTION_TOKEN` may not expand correctly in bash/curl. Always fetch via Python first:
```bash
NOTION_TOKEN=$(python3 -c "import os; print(os.environ['NOTION_API_TOKEN'])")
```
Then use `$NOTION_TOKEN` in subsequent curl commands within the same shell.
